   ...
   //port.connect(io)
    ...
    io
      ...
      .on( 'connection', function ( socket ) {
        ...
        socket.on('pulisciDisegno', function( user_map ) {

          console.log('server arrivata richiesta pulisciDisegno per utente: ' + user_map)
          pixelart.pulisciDisegno(user_map,socket,'pulisciDisegno');

        })
        socket.on('aggiungiPixel', function( user_map ) {

          console.log('server arrivata richiesta aggiungi figurina album per utente: ' + user_map)
          pixelart.aggiungiPixel(user_map,socket,'searchDisegnoUtente');

        })
        socket.on('eliminaPixel', function( user_map ) {

          console.log('server arrivata richiesta elimina figurina album per utente: ' + user_map)
          pixelart.eliminaPixel(user_map,socket,'searchDisegnoUtente');

        })
        socket.on('searchDisegnoUtente', function( user_map ) {

          console.log('server arrivata richiesta figurine album per utente: ' + user_map)
          pixelart.searchPerUtente(user_map,socket,'searchDisegnoUtente');

        })
        socket.on('searchAnagraficaDisegno', function( user_map ) {

          console.log('server arrivata richiesta elecno  per cognome: ' + user_map)
          pazienti.searchCognome(user_map,socket,'searchAnagraficaDisegno');

        })

        socket.on('aggiungiFigurina', function( user_map ) {

          console.log('server arrivata richiesta aggiungi figurina album per utente: ' + user_map)
          album.aggiungiFigurina(user_map,socket,'searchAlbumUtente');

        })
        socket.on('eliminaFigurina', function( user_map ) {

          console.log('server arrivata richiesta elimina figurina album per utente: ' + user_map)
          album.eliminaFigurina(user_map,socket,'searchAlbumUtente');

        })
        socket.on('searchFigurineAlbum', function( user_map ) {

          console.log('server arrivata richiesta figurine per album : ' + user_map)
          //album.searchPerUtente(user_map,socket,'searchAlbumUtente');
          album.searchFigurineAlbum(user_map,socket,'searchFigurineAlbum');
        })
        socket.on('searchAlbumUtente', function( user_map ) {

          console.log('server arrivata richiesta figurine album per utente: ' + user_map)
          //album.searchPerUtente(user_map,socket,'searchAlbumUtente');
          album.searchAlbumUtente(user_map,socket,'searchAlbumUtente');
        })
        socket.on('searchAnagraficaAlbum', function( user_map ) {

          console.log('server arrivata richiesta elecno  per cognome: ' + user_map)
          pazienti.searchCognome(user_map,socket,'searchAnagraficaAlbum');
