...
const userHTMLTemplate = `
      <div class="row">
        ...
        <input type="submit" value="Continua per registrare/modificare il paziente" class="btn">
        </form>
        </fieldset>
      </div>
       </div>
...
<form id="search">
  <div class="form-group">
    ...
    <button id="nuovoPaziente" type="button" class="btn-sm btn-link" >Registra un nuovo nominativo</button>
</div>
</form>
</div>
...
<div id="dtlPazienti" class="newread-sub">  </div>
</div>
`;
...
`;
  document.getElementById('headerpaziente' ).innerHTML = headePazientePulitoHTML
  document.querySelector( '.container-centrale' ).innerHTML = userHTMLTemplate;
  //document.querySelector( '.container-destro' ).innerHTML = laterale;
  const aa = `
        ${elencocomuni.map(row => `
        <option value="${row.Descrizione}">${row.Descrizione}</option>`
        ).join('')}`
  ...
  //const btn_cf = document.getElementById('calcolaCF1')
  //btn_cf.addEventListener('click',fncalcolaCF,false);
  document.querySelector( '.newread-sub' ).innerHTML = anagraficalayout_html;
};
